...
export class SelectService implements ISelectService {
  ...
  getStyles(): string {
    return `
      /* <![CDATA[ */
        .selected-element-group .selected-element-border {
          pointer-events: none;
        }
      /* ]]> */
    `;
  ...
  private setGroup(svg: Svg): void {
    if (!this.group) {
      this.group = svg.findOne('g.selected-element-group');
      if (!this.group) this.group = svg.group().addClass('selected-element-group');
    }
  }
